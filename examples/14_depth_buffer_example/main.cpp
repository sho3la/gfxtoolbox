...
init()
{
	...
	projection = glm::perspective(glm::radians(45.0f), (float)scrn_width / (float)scrn_height, near_plane, far_plane);
...
render_depthmap()
{
	...
	gfx_backend->setGPUProgramMat4(depth_gpu_program, "view", view);
	gfx_backend->setGPUProgramMat4(depth_gpu_program, "projection", projection);
...
render()
{
	...
	gfx_backend->setGPUProgramFloat(gpu_program, "near_plane", near_plane);
	gfx_backend->setGPUProgramFloat(gpu_program, "far_plane", far_plane);
	gfx_backend->bindTexture2D(depth_frame_buffer->GetTexture());

	// render frame quad
	gfx_backend->draw_indexed(gfx::GFX_Primitive::TRIANGLES, quad_gpu_mesh_id, 6);
}
...
main()
{
	// initialize gfx
	// ---------------------------------------
	gfx_backend->init("depth texture example", scrn_width, scrn_height);
