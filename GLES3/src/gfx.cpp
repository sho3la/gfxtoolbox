...
namespace gfx
{
	...
	_getGLErrorString(GLenum error)
	{
		switch (error)
		{
		case GL_NO_ERROR:
	...
	GFX::init(const char* window_title, int window_width, int window_height)
	{
		...
		glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_ANY_PROFILE); // OpenGL ES 3.2
	...
	GFX::start()
	{
		...
		// render loop
#ifdef __EMSCRIPTEN__
		emscripten_set_main_loop_arg(GFX::render_frame, this, 60, 1);
#else
		while (!glfwWindowShouldClose(window))
		{
			GFX::render_frame(this);
		}
#endif
	}
	...
	void
	GFX::render_frame(void* arg)
	{
		GFX* self = static_cast<GFX*>(arg);
		glfwPollEvents();

		if (self->m_renderCallback)
			self->m_renderCallback();
